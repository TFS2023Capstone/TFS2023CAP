...
namespace HiddenWorld.Helpers
{
    ...
    public class CursorController : MonoBehaviour
    {
        ...
        public enum CursorState { TPS, Menu, TPSVisable };

        [Header("Settings")]
        [Tooltip("The state to start the cursor in in this scene")]
        public CursorState startState = CursorState.TPS;
        ...
        {
            if (cursorState == CursorState.TPS)
            {
                Cursor.lockState = CursorLockMode.Locked;
                Cursor.visible = false;
            }
            else if (cursorState == CursorState.TPSVisable)
            {
